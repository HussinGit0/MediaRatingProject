namespace MediaRatingProject.Server
{    
    using System.Net;
    using System.IO;
    ...
    internal class APIListener
    {
        ...
        private void LoginUser(string body)
        {
            ...
            foreach (BaseUser user in this.UserDB.Users)
            {
                if (user.Username == loginRequest.Username && user.Password == loginRequest.Password)
                {
                    Console.WriteLine($"Sucessfully logged in as {user.Username}.");
                    return;
                }
            }

            Console.WriteLine("Error in name or password.");
        }
    }
}
